...
class Screen(BaseClient):
    ...
    def handle_chat(self, client=None, message=None):
        if self.inputs[2].return_pressed:
            if self.inputs[2].text.strip() != '':
    ...
    def draw_chat_all(self):
        ...
        if total > 4:
            ...
            self.chat_all_scroll = 0
        pygame.draw.rect(self.screen, Color.LIGHT_GOLD,
                         self.chat_all_rect, border_radius=5)
        ...
        for i in range(4):
            ...
            # Ajuste para exibir de baixo para cima
            y = self.chat_all_rect.bottom - 18 - (i * 18)
            x = self.chat_all_rect.right - 25 - \
                surface.get_width(
                ) if msg['is_self'] else self.chat_all_rect.left + 25
