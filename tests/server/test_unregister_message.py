def test_complain_about_extra_arguments(server):
    addr = ('127.0.0.1', 5000)
    server.parse_server_message('REGISTER', ['Alice'], addr)

    response = server.parse_server_message('UNREGISTER', ['extra'], addr)
...
def test_error_on_client_unregistred(server):
    ...
    response = server.parse_server_message('UNREGISTER', [], addr)
