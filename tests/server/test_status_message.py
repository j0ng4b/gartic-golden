def test_complain_about_client_unregistred(server):
    ...
    response = server.parse_server_message('STATUS', [], addr)
    assert response == 'Cliente não registrado'


def test_complain_about_extra_arguments(server):
    addr = ('127.0.0.1', 6001)
    server.parse_server_message('REGISTER', ['Big Smoke'], addr)

    response = server.parse_server_message('STATUS', ['arg'], addr)
...
def test_output_format(server):
    addr = ('127.0.0.1', 6001)
    server.parse_server_message('REGISTER', ['Pikachu'], addr)
